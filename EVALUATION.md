...
| Room for three guests | Matches rooms with capacity >= 3 | Passed by deterministic room service |
...
| Frontend initial state | Renders welcome message, question input, and availability controls | Passed |
| Frontend-to-backend flow | React API client uses `/api/chat` and `/api/availability` | Verified with live curl/API checks |
...
Observed result: **9 automated tests passed** (8 backend, 1 frontend) and the production React bundle built successfully.
