...
export async function answerChat({ question, history = [], availability }) {
  ...
  const availabilityIntent = /availability|vacan|rooms?\s+avail(?:able|ble|bl)?|avail(?:able|ble|bl)\s+rooms|room(?:s)?\s+(?:for|on|from|between|during|avail(?:able|ble|bl)?)|(?:for|with)\s+\d+\s+(?:adult|guest|people|person|travell?er|occupant)|(?:for|with)\s+(?:one|two|three|four|five|six)\s+(?:adult|guest|people|person)|(?:family|group|couple)\s+(?:room|suite|stay)\s+(?:for|on|from|between|during)|check[- ]?in\s+(?:date|on)|check[- ]?out\s+(?:date|on)|booking|book|reserve|reservation|night(?:s)?|dates?\s+(?:from|for|on)|stay/.test(q);
