...
If the model key is missing, the provider times out, returns an error, exceeds quota, or produces an unusable response, the backend falls back to a deterministic answer from the hotel knowledge base. If the frontend API call fails, the chat displays a dismissible error without losing the existing conversation. If MongoDB is unavailable in this assignment environment, the application uses the clearly labelled demo dataset; production should fail closed or use a formally managed read replica rather than silently relying on demo data. Request IDs are returned to help correlate a user-visible failure with server logs. Availability errors remain separate from AI errors so the guest receives a specific next step.
...
The AI layer should gain provider fallback, timeout and retry policies with backoff, model/version tracking, prompt and knowledge-base versioning, cost limits, prompt-injection testing, and a red-team evaluation set. I would also add browser-level end-to-end tests, accessibility checks, real MongoDB integration tests, and contract tests for the PMS provider. Finally, I would make the no-reservation/demo state impossible to confuse with a real booking by using explicit product copy and a separate production booking workflow.
